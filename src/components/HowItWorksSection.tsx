      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-12">
          {steps.map((step, index) => (
            <div 
              key={index} 
              className="bg-edubridge-bg rounded-xl p-6 shadow-md card-hover"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="flex flex-col items-center text-center">
                <div className="w-16 h-16 rounded-full bg-edubridge-blue text-white flex items-center justify-center mb-4">
                  <step.icon size={28} />
                </div>
                <div className="relative mb-6">
                  <h3 className="text-xl font-bold mb-2">{step.title}</h3>
                  {index < steps.length - 1 && (
                    <div className="hidden lg:block absolute top-1/2 left-full w-full h-0.5 bg-gray-300 -translate-y-1/2">
